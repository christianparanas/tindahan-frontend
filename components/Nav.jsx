...
export default function Nav(props) {
  ...
  const [navOverlayClickOutside, setNavOverlayClickOutside] = useState(
    "navOverlay-clickOutside"
  );
  ...
  useEffect(() => {
    ...
    if (cookies.user) {
      ...
      axios
        .post(process.env.BACKEND_BASEURL + "/user/cart", {
  ...
  const changeQuan = (qty, id, cart_qty, stock) => {
    ...
    if (qty == 1) {
      ...
      if (cart_qty < stock) {
        axios
          .post(process.env.BACKEND_BASEURL + "/user/updatecartqty", {
            order: "add",
            id: id,
          })
          .then(async (res) => {
      ...
      if (cart_qty <= 1) {
        axios
          .post(process.env.BACKEND_BASEURL + "/user/delcartitem", {
            id: id,
          })
          .then(async (res) => {
            if (res.status == 200) {
    ...
    <div className="nav">
      ...
      <div className={upperNav}>
        ...
        <div className="upper_nav_svgs">
          ...
          >
            <defs></defs>
            <path className="cls-1" d="M7 15h51M7 32h43M7 49h51"></path>
          </svg>
        ...
        <div>
          <Link href="/">
            ...
            >
              ...
              <path d="M12,17c2.206,0,4-1.794,4-4s-1.794-4-4-4c-2.206,0-4,1.794-4,4S9.794,17,12,17z M12,11c1.103,0,2,0.897,2,2s-0.897,2-2,2 c-1.103,0-2-0.897-2-2S10.897,11,12,11z"></path>
            </svg>
          </Link>
        </div>
        <div>
          <svg
            ...
            viewBox="0 0 24 24"
          >
            <path d="M20,11V4h-7l2.793,2.793l-4.322,4.322C10.49,10.416,9.294,10,8,10c-3.309,0-6,2.691-6,6s2.691,6,6,6s6-2.691,6-6 c0-1.294-0.416-2.49-1.115-3.471l4.322-4.322L20,11z M8,20c-2.206,0-4-1.794-4-4s1.794-4,4-4s4,1.794,4,4S10.206,20,8,20z"></path>
          </svg>
        </div>
        <div>
          <svg
            ...
            viewBox="0 0 24 24"
          >
            <path d="M12,2C8.691,2,6,4.691,6,8c0,2.967,2.167,5.432,5,5.91V17H8v2h3v2.988h2V19h3v-2h-3v-3.09c2.833-0.479,5-2.943,5-5.91 C18,4.691,15.309,2,12,2z M12,12c-2.206,0-4-1.794-4-4s1.794-4,4-4c2.206,0,4,1.794,4,4S14.206,12,12,12z"></path>
          </svg>
        </div>
        <div>
          <Link href="/account">
      ...
      <motion.div className={navOverlay}>
        ...
        <div className="navOptions">
          <Link href="/products">
            <div className="navOp collections">Products</div>
          </Link>
          ...
          {hascartItems ? (
            <div className="bucket">
              {cartItems.map((val, key) => {
                return (
                  <div key={key} className="itemCart">
                    <img
                      src={`https://res.cloudinary.com/christianparanas/image/upload/v1617305941/Ecommerce/Products/${val.cart_p_image}`}
                      alt="product image"
                    />
                    <div className="itemCart_details">
                      <div className="ab">{val.cart_p_name}</div>
                      <div className="aa">
                        ₱{val.cart_p_price.toLocaleString()}
                      </div>
              ...
              <div className="review_cart">
                ...
                <Link href="/cart">
                  <div className="review_cartBtn">Review My Cart</div>
                </Link>
              </div>
            </div>
          ) : (
